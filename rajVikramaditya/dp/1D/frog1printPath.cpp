...
//if question is asking 1based indexing then print i+1 else not required
void printPath(vi &arr, int n, vi &dp) {
	int i = 0;
	cout<<i+1<<" ";
	while(i < n-1) {
		if(dp[i]-abs(arr[i]-arr[i+1]) == dp[i+1]) i += 1;
		else i += 2;
		cout<<i+1<<" ";
	}
	cout<<endl;
